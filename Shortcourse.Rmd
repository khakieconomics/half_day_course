...
biased, or having a poor understanding of how exactly the parameter influences the model. 
...
In theory, our posterior $p(\theta | y)$ is an abstract distribution; in practice, it's a matrix of data, where each column 
corresponds to a parameter, and each row a draw from $p(\theta | y)$. For instance, the first five rows of 
our posterior matrix might look like: 
...
different out-of-sample likelihoods for our out of sample datapoint $y_{j} = 1,x_{j} = 1$. This is illustrated
...
The density of the out-of-sample data across posterior draws is known as the (out of sample) _log posterior density_ of
a datapoint. Unlike the out of sample likelihood, we are uncertain of its value and need to perform inference. To illustrate 
...
## A quick introduction to Hamiltonian Monte Carlo
...
<!--chapter:end:01-An_introduction_to_Stan.Rmd-->


# Modern Statistical Workflow  {#msw}

<!--chapter:end:02-modern_statistical_workflow.Rmd-->


# A more difficult model {#vsb}

<!--chapter:end:03-a_more_difficult_model.Rmd-->
